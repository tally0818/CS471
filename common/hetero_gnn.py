...
class HeteroGNNEncoder(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, n_layers: int = 2, dropout: float = 0.5, use_relu: bool = True):
        ...
        self.w_embed = nn.Parameter(
            torch.zeros(size=(input_dim, hidden_dim)),
            requires_grad=True
        )
        self.w_classify = nn.Parameter(
            torch.zeros(size=((2 ** (self.k + 1) - 1) * hidden_dim, output_dim)),
            requires_grad=True
        )
